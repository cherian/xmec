...
for ($i = 0; $i < $res['count']; $i++) {
	echo "<TR". (($i%2)?" bgcolor=\"#CFDDD1\">":">");
   	...
   	echo "<TD><A href=Edit onClick=\"javascript:open('profile_admin.php?.s=1&id=",rawurlencode($res[$i]->id),"','','scroll=yes,scrollbars=yes');return false;\" class=slink>", htmlentities($res[$i]->name), "</a>\n";
   	echo "&nbsp;&nbsp;<A href=Preferences onClick=\"javascript:open('preferences.php?.s=1&id=",rawurlencode($res[$i]->id),"','','scroll=yes,scrollbars=yes');return false;\" class=slink>pref</a></TD>\n";
