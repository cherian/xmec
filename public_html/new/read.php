...
////////////////////////////////////////////////////////////////////////////////
// $Id: read.php,v 1.81 2002/01/22 05:37:15 brianlmoon Exp $
  ...
  if(@is_array($ids)){
    ...
    $SQL="select id, concat(first_name, ' ', middle_name, ' ', last_name) as name, personal_email, signature from xmec_user where id in (".implode(",", $ids).")";
    $q->query($DB, $SQL);
    $rec=$q->getrow();
    While(is_array($rec)){
      $users[$rec["id"]]=$rec;
...
<table class="PhorumListTable"  width="<?php echo $ForumTableWidth; ?>" cellspacing="0" cellpadding="3" border="0">
<tr>
...
<tr>
    <td <?php echo bgcolor($ForumTableBodyColor2); ?> valign="TOP"><table class="PhorumListTable" width="100%" cellspacing="0" cellpadding="5" border="0">
<tr>
  ...
  while(is_array($head_row) && is_array($body_row)){
    if($head_row["id"]==$body_row["id"]){
      ...
      if($head_row["userid"]>0 && isset($users[$head_row["userid"]])){
        ...
        $email=$user["email"];
        $host="";
        $author = "<a class=link href=\"$forum_url/profile.$ext?f=$ForumId&id=$head_row[userid]$GetVars\">$author</a>";
        // replace sig
        $sig=$user["signature"];
      }
      else{
