...
for ($i = 0; $i < $res['count']; $i++) {
	echo "<TR". (($i%2)?" bgcolor=\"#CFDDD1\">":">");
   	...
   	echo "<TD><A href=Edit onClick=\"javascript:open('profile_admin.php?.s=1&id=",rawurlencode($res[$i]->id),"','','scroll=yes,scrollbars=yes');return false;\" class=flink>", htmlentities($res[$i]->name), "</a>\n";
   	echo "&nbsp;&nbsp;<A href=Preferences onClick=\"javascript:open('admin_preferences.php?.s=1&id=",rawurlencode($res[$i]->id),"','','width=600,height=480,scroll=yes,scrollbars=yes');return false;\" class=link>pref</a>\n";
